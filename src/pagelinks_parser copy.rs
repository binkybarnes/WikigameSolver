...
pub fn build_pagelinks(
    ...
    for line in decompressed_reader.lines() {
        ...
        if line.starts_with(PREFIX) {
            ...
            let Some(end_paren) = line.rfind(')') else {
                continue;
            };
            let trimmed = &line[start_paren + 1..end_paren];

            for tuple in trimmed.split("),(") {
                counter += 1;
                let fields: Vec<&str> = tuple.split(',').collect();
                if fields.len() != 3 {
                    println!("invalid tuple: {:?}", tuple);
                    std::process::exit(1);
                }
                if fields[1] == "0" {
                    let page_id_from: u32 = fields[0].parse().expect("Invalid page_id_from");
                    let mut page_id_to: u32 = fields[2].parse().expect("Invalid page_id_to");
